...
export default {
     ...
     * @param {Object} env Your environment variables 
    ...
    async fetch(request, env, context) {
        try {
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                console.info('Got a websocket request here pal.')
...
async function WSHandler(request, env, context) {
    ...
    const rstream = new ReadableStream({
        start(controller) {
            ...
            if (earlyDataHeader) {
                console.log('Got sec-websocket-protocol', earlyDataHeader); // debug-log
                try {
        ...
        cancel(reason) {
            ...
            console.info(`ReadableStream was canceled, due to ${reason}`);
            rstreamCancel = true;
            if (webSocket.readyState === WS_READY_STATE_OPEN || webSocket.readyState === WS_READY_STATE_CLOSING) {
                webSocket.close();
            }
    ...
    rstream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            if (isDNS) {
                handleUDPOutBound(webSocket, vlessResponseHeader, rawClientData);
                return;
            }
            handleTCPOutBound(remoteSocketWrapper, address, port, rawClientData, webSocket, vlessResponseHeader);
        },
        close() {
            console.info('ReadableWebSocketStream was closed.');
        },
        abort(reason) {
            console.warn(`ReadableWebSocketStream was aborted, due to ${reason}`);
        }
    }))
    .catch((error) => {
...
function vlessHeaderParser(buffer) {
    ...
    // 0X02 UDP
    // 0X03 MUX
    console.info(`Protocol is ${protocol}`)
    if (protocol === 1) {
        // Default
    }
    else if (protocol === 2) {
...
async function handleUDPOutBound(webSocket, vlessResponseHeader, rawClientData) {
    console.log('Got an UDP request here pal.') // debug-log
    ...
    transform.readable.pipeTo(new WritableStream({
        async write(chunk) {
            console.log('Sending DNS query to 1.1.1.1')
            ...
            if (webSocket.readyState === WS_READY_STATE_OPEN) {
                console.info(`DOH success and DNS message length is ${UDPSize}`);
...
async function remoteSocketToWS(TCPSocket, webSocket, vlessResponseHeader, retry = null) {
    ...
    await TCPSocket.readable.pipeTo(new WritableStream({
        ...
        async write(chunk, controller) {
            ...
            if (webSocket.readyState !== WS_READY_STATE_OPEN) {
                console.error(`webSocket.readyState is closed.`);
        ...
        close() {
            console.warn(`TCPSocket.readable is close with incoming data is ${hasIncomingData}`);
        },
        abort(reason) {
            console.error(`TCPSocket connection aborted due to ${reason}`);
        }
    }))
    .catch((error) => {
        console.error(`TCPSocket have exception: ${error.stack || error}`);
        if (webSocket.readyState === WS_READY_STATE_CLOSING || webSocket.readyState === WS_READY_STATE_OPEN) {
            webSocket.close();
        }
    })
